...
const ProjectListEmployee = () => {
  ...
  const filterCards = event => {
    ...
    const filteredProjects = fliterprojects.filter(project => (`${project.name} ${project.location}`.toLowerCase().includes(value)));
    setProjects(filteredProjects);
  }
return (
  <div className="project-list">
    <h1 style={{textAlign:"center"}}>Projects</h1>
    <input className="search-box" onInput={filterCards} placeholder="Search Projects"/>
