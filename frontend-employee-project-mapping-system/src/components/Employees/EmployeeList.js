...
const EmployeeList = () => {
  ...
  const filterCards = event => {
    ...
    const filteredEmployees = fliteremployee.filter(employee => (`${employee.name} ${employee.email}`.toLowerCase().includes(value)));
    setEmployee(filteredEmployees);
  }
return (
  <div className="project-list">
    <h1 style={{textAlign:"center"}}>Employees</h1>
    <span>Search  </span><input className="search-box" onInput={filterCards} placeholder="Search Employee"/> 
